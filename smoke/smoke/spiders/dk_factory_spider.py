...
class DKBaseSpier(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.FormRequest(LOGIN_URL, formdata=LOGIN_FORM_DATA, callback=self.after_login)

    def after_login(self, response):
        for idx, page in enumerate(self.pages):
            yield scrapy.Request(url=f"{self.target_url}{page}", callback=self.parse, cb_kwargs={"page": idx})
